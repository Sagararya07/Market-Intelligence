...
export default function DataAnalytics() {
  ...
  const fetchResults = async (filename: string) => {
    ...
    try {
      ...
      const data = await api<any>(\`/imports/file/\${encodeURIComponent(filename)}\`);
  ...
  const downloadCSV = () => {
    ...
    link.download = \`Lumora_\${selectedFile.replace(/[^a-zA-Z0-9]/g, "_")}.csv\`;
    link.click();
  };
            ...
            <div className="space-y-2">
              {files.map(f => (
                <button
                  ...
                  className={\`w-full text-left p-3 rounded-lg text-sm transition-colors border \${selectedFile === f.name ? 'border-theme-blue bg-theme-blue/5 text-theme-blue font-medium' : 'border-slate-100 hover:border-slate-200 text-slate-600 hover:bg-slate-50'}\`}
                >
        ...
        <div className="flex-1">
          {isLoadingResults && (
            <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-slate-200 flex flex-col items-center justify-center">
              ...
              <p className="text-slate-500 text-sm">Please wait while we fetch the records for {selectedFile}.</p>
            </div>
          )}

          {!isLoadingResults && selectedFile && results.length > 0 && (
...
fs.writeFileSync('client/src/pages/DataAnalytics.tsx', code);
console.log('Done DataAnalytics!');
