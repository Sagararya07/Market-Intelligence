...
dashboardRouter.get("/", async (req: AuthRequest, res, next) => {
  try {
    ...
    const [accounts, signals, requirements, opportunities, assessments] = await Promise.all([
    ...
    res.json({ accounts, signals, requirements, hotOpportunities: byClass.HOT ?? 0, warmOpportunities: byClass.WARM ?? 0, qualifiedAccounts: assessments.filter(a=>a.overallScore>=70).length, opportunities, averageIcp: avg });
  } catch (e) { next(e); }
});
