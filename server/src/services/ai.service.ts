...
export async function analyzeAccountsBatch(accounts: any[]) {
  ...
  for (let i = 0; i < accounts.length; i += BATCH_SIZE) {
    ...
    const chunkPromises = chunk.map(async (account) => {
      ...
      - Also extract the date when the requirement or budget was announced or declared into 'declaredAt' in YYYY-MM-DD format. CRITICAL: If no specific date is mentioned in the text, you MUST return null. Do NOT hallucinate a date.
      - Also provide the 'sourceUrl' where this specific requirement was found. CRITICAL: If there is no specific URL (like a news article or press release) provided in the context, you MUST return null. Do NOT use the generic company website, and do NOT use example URLs.
      ...
      You MUST return ONLY a raw JSON object with this exact structure (no markdown tags, no explanations, no example URLs):
      {
        ...
        "requirement": { "title": "string", "category": "string", "description": "string", "urgency": "HIGH|MEDIUM|LOW", "confidence": 0.85, "budgetMin": 50000, "budgetMax": 150000, "budgetCurrency": "USD", "declaredAt": null, "sourceUrl": null }
      }
      `;
